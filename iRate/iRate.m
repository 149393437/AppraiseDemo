...
-(void)showAlertWithStoreID:(int)storeID BundleID:(NSString*)bundleID  MessageTitle:(NSString*)title Message:(NSString*)message rateTitle:(NSString*)rateTitle waitTitle:(NSString*)waitTitle cancelTitle:(NSString*)cancelTitle Block:(void(^)(int))a{
    self.alertBlock=a;
    sharedInstance.appStoreID = storeID;
    sharedInstance.applicationBundleID = bundleID;
    sharedInstance.onlyPromptIfLatestVersion = NO;
    sharedInstance.previewMode = YES;
    
    //弹出警告框的文字说明，如果不设置也可以是默认的
    sharedInstance.messageTitle=title;
    sharedInstance.message=message;
    sharedInstance.rateButtonLabel=rateTitle;
    sharedInstance.remindButtonLabel=waitTitle;
    sharedInstance.cancelButtonLabel=cancelTitle;
}
...
{
    if (self.alertBlock) {
        self.alertBlock((int)buttonIndex);
    }
